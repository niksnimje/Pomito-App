...
function Home() {
  ...
  const [showBreakResetModal, setshowBreakResetModal] = useState(false); 
  const { theme } = useContext(ThemeContext);
  const { running, resetTimer } = useContext(TimeContext2);
  ...
  const logSessionEvent = (type, duration = null) => {
    const prev = JSON.parse(localStorage.getItem('sessionLog')) || { sessions: [] };
    const entry = {
      id: Date.now(),
      type,
    ...
    localStorage.setItem('sessionLog', JSON.stringify({ sessions: updated }));
  };

  const toggleTimer = () => {
    if (running === true) {
      setshowBreakResetModal(true);
      return;
    }
    if (!runningg) {
      logSessionEvent("start", duration);
    }
    setrunningg(!runningg);
  };

  const confirmDeepReset = () => {
    resetTimer(); 
    logSessionEvent("reset");
    setshowBreakResetModal(false);
    logSessionEvent("start", duration);
    setrunningg(true); 
  };
      ...
      {showModal && (
        <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className={clsx(
            ...
            <h2 className="text-lg font-semibold mb-4">Are you sure you want to end the session?</h2>
        ...
        <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className={clsx(
            ...
            <h2 className="text-lg font-semibold mb-4">
              A Break session is already running.<br /> Do you want to reset and start Deep Work Time?
            </h2>
            <div className="flex justify-center space-x-4">
              <button 
              ...
              <button 
                onClick={() => setshowBreakResetModal(false)} 
                className={clsx(
                  'px-6 py-2 rounded-lg',
                  theme === 'obsidia' && 'bg-[#44403c] text-white',
                  theme === 'one-dark' && 'bg-[#333842] text-white',
